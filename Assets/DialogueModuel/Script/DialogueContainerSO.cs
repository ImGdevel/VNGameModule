...
namespace MeetAndTalk
{
    ...
    {
        public List<NodeLinkData> NodeLinkDatas = new List<NodeLinkData>(); // 노드 간 연결 데이터를 저장하는 리스트
        public List<DialogueChoiceNodeData> DialogueChoiceNodeDatas = new List<DialogueChoiceNodeData>(); // 대화 선택 노드 데이터를 저장하는 리스트
        public List<DialogueNodeData> DialogueNodeDatas = new List<DialogueNodeData>(); // 대화 노드 데이터를 저장하는 리스트
        public List<EndNodeData> EndNodeDatas = new List<EndNodeData>(); // 종료 노드 데이터를 저장하는 리스트
        public List<StartNodeData> StartNodeDatas = new List<StartNodeData>(); // 시작 노드 데이터를 저장하는 리스트

        [Header("Pro Version Node")]
        public List<TimerChoiceNodeData> TimerChoiceNodeDatas = new List<TimerChoiceNodeData>(); // 타이머 선택 노드 데이터를 저장하는 리스트
        public List<EventNodeData> EventNodeDatas = new List<EventNodeData>(); // 이벤트 노드 데이터를 저장하는 리스트
        public List<RandomNodeData> RandomNodeDatas = new List<RandomNodeData>(); // 랜덤 노드 데이터를 저장하는 리스트
        public List<CommandNodeData> CommandNodeDatas = new List<CommandNodeData>(); // 명령 노드 데이터를 저장하는 리스트
        public List<IfNodeData> IFNodeDatas = new List<IfNodeData>(); // 조건 노드 데이터를 저장하는 리스트
