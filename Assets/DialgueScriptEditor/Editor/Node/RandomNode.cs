...
namespace DialogueSystem.Nodes
{
    ...
    public class RandomNode : BaseNode
    {
        ...
        public Port AddOutputPort(BaseNode _basenote, DialogueNodePort _dialogueNodePort = null)
        {
        ...
        public override void SetValidation()
        {
            ...
            if (dialogueNodePorts.Count < 1) {
                error.Add("You need to add more Output");
            }
            else {
                for (int i = 0; i < dialogueNodePorts.Count; i++) {
                    if (!dialogueNodePorts[i].MyPort.connected) {
                        error.Add($"Random Node : {i} does not lead to any node");
                    }
                }
            }
