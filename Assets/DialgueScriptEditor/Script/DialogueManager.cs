...
namespace DialogueSystem
{
    public class DialogueManager : DialogueGetData
    {
        ...
        {
            dialogueScript = dialogue; // 대화 스크립트 가져옴
        }
        ...
        public void StartDialogue(DialogueScript dialogue)
        {
            ...
            if (dialogueScript.StartNodeDatas.Count == 1)  // 시작노드가 한개인 경우
                CheckNodeType(GetNextNode(dialogueScript.StartNodeDatas[0]));
            else { // 시작노드가 여러개인 경우
                CheckNodeType(GetNextNode(dialogueScript.StartNodeDatas[Random.Range(0, dialogueScript.StartNodeDatas.Count)]));   
            }
        ...
        public void StartDialogue(string ID)
        {
            ...
            bool withID = false;
            for(int i = 0; i < dialogueScript.StartNodeDatas.Count; i++)
            {
                if(dialogueScript.StartNodeDatas[i].startID == ID)
                {
                    CheckNodeType(GetNextNode(dialogueScript.StartNodeDatas[i]));
                    withID = true;
                }
            }
            if (!withID)
            {
                if (dialogueScript.StartNodeDatas.Count == 1) CheckNodeType(GetNextNode(dialogueScript.StartNodeDatas[0]));
                else { CheckNodeType(GetNextNode(dialogueScript.StartNodeDatas[Random.Range(0, dialogueScript.StartNodeDatas.Count)])); }
            }
