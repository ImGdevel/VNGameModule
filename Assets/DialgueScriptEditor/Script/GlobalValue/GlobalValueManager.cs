...
namespace DialogueSystem.GlobalValue
{
    public class GlobalValueManager : ScriptableObject
    {
        ...
        public static string k_LocalizationManagerPath {
            get {
                ...
                var path = Path.Combine(Path.GetDirectoryName(scriptFilePath), "../../Resources/GlobalValue.asset");
    ...
    static class GlobalValueManagerIMGUIRegister
    {
        ...
        {
            var provider = new SettingsProvider("Project/Dialogue System/Global Value", SettingsScope.Project)
            {
                label = "Glabal Value Manager",
                guiHandler = (searchContext) =>
                {
                    EditorGUILayout.HelpBox("Tu dam jaki?opis", MessageType.Info, true);
